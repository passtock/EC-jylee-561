...
void ICAP_init(PinName_t pinName) {
    ...
    // 3. Enable Counter - @ end of initiation
    ...
    TIMx->CCER &= ~(0b1010 << 4 * (ICn - 1));  // CCy(Rising)
    // TIMx->CCER |=  (0b0010 << 4*(ICn-1); 	// CCy(Falling)
    ...
    TIMx->DIER |= 1 << ICn;      // Capture/Compare Interrupt Enable
